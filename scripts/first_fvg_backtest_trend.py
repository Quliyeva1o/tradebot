"""Variant of scripts/first_fvg_backtest_wide_sl.py adding a HIGHER-TIMEFRAME
TREND FILTER: only take the day's FVG if its direction agrees with the trend
on TREND_TF_MINUTES (15=M15, 60=H1, 1440=D1), else skip the day entirely --
no counter-trend trades. Trend = close vs SMA(TREND_SMA_PERIOD) on that
higher timeframe, evaluated on the last fully-closed higher-TF bar strictly
before the 00:00 NY session start (no lookahead). BACKTEST ONLY per the
user's request -- not wired into strategy/midnight_fvg.py or the live bot.
Everything else (entry/SL/TP rules, including the wide-SL change) is
identical to first_fvg_backtest_wide_sl.py.

Wide-SL docstring (unchanged, carried over):

Variant of scripts/first_fvg_backtest.py testing a WIDER stop-loss: instead
of the displacement (middle) candle's own wick, SL = the wick of the candle
BEFORE the displacement candle (the FVG's first/start candle) -- i.e. one
candle earlier. Requested after analysis showed 12.3% of TP trades came
within >=80% of the original (tighter) risk distance of SL before reversing
(a near-miss "stop hunt" pattern) and 38.5% within >=50%; the hypothesis is
that a wider SL would survive more of those near-misses. Since TP is a fixed
R-multiple of risk_dist, a wider SL also means a wider TP price-distance --
this changes which trades reach TP vs SL, not just how often SL alone gets
hit. Compare this script's summary() output against the original's to see
the net effect. See docstring below (unchanged from the original except for
this SL rule) for the rest of the strategy's rules.
...
TREND_TF_MINUTES = 1440  # 15=M15, 60=H1, 1440=D1 -- change and rerun for each variant
TREND_SMA_PERIOD = 20
TRADES_OUT = f"artifacts/first_fvg_trades_trend_{TREND_TF_MINUTES}m.csv"
...
def _floor_ts(ts: datetime, tf_minutes: int) -> datetime:
    """Floors an NY-aware timestamp to its higher-timeframe bucket start.
    D1 (tf_minutes>=1440) buckets by calendar date; H1/M15/etc bucket by
    minutes-since-midnight, which sidesteps any cross-day/DST arithmetic.
    """
    if tf_minutes >= 1440:
        return datetime(ts.year, ts.month, ts.day, tzinfo=ts.tzinfo)
    minutes_since_midnight = ts.hour * 60 + ts.minute
    floored = (minutes_since_midnight // tf_minutes) * tf_minutes
    h, m = divmod(floored, 60)
    return datetime(ts.year, ts.month, ts.day, h, m, tzinfo=ts.tzinfo)


def build_higher_tf_bars(m1_bars: list[NyBar], tf_minutes: int) -> list[NyBar]:
    """Resamples M1 bars into tf_minutes-bars (NY wall-clock buckets)."""
    buckets: dict[datetime, list[NyBar]] = {}
    for b in m1_bars:
        buckets.setdefault(_floor_ts(b.ts, tf_minutes), []).append(b)
    out = [
        NyBar(ts=ts, open=grp[0].open, high=max(x.high for x in grp), low=min(x.low for x in grp), close=grp[-1].close)
        for ts, grp in buckets.items()
    ]
    out.sort(key=lambda b: b.ts)
    return out


def build_trend_lookup(higher_tf_bars: list[NyBar], sma_period: int) -> dict[datetime, str]:
    """Maps each higher-TF bucket's ts to "UP"/"DOWN" (close vs a trailing
    SMA(sma_period) computed from bars strictly BEFORE that bucket, so a
    session at time T only ever consults trend info known before T -- no
    lookahead). Bars before the SMA has enough warmup get no entry (session
    then skips for lack of a trend value).
    """
    trend: dict[datetime, str] = {}
    closes: list[float] = []
    for b in higher_tf_bars:
        if len(closes) >= sma_period:
            sma = sum(closes[-sma_period:]) / sma_period
            trend[b.ts] = "UP" if b.close > sma else "DOWN"
        closes.append(b.close)
    return trend


def trend_as_of(trend_lookup: dict[datetime, str], higher_tf_bars: list[NyBar], before_ts: datetime) -> str | None:
    """Returns the trend value of the last higher-TF bucket that closed
    strictly before before_ts (typically the session's 00:00 NY start).
    higher_tf_bars is ts-sorted, so binary-search the cut point instead of a
    linear scan -- matters once this is called once per trading day against
    an M15-resolution (~4yr * ~35k bars) higher_tf_bars list.
    """
    import bisect
    idx = bisect.bisect_left(higher_tf_bars, before_ts, key=lambda b: b.ts)
    for b in reversed(higher_tf_bars[:idx]):
        if b.ts in trend_lookup:
            return trend_lookup[b.ts]
    return None
...
def process_session(
    ...
    # TREND FILTER: only take the FVG if it agrees with the higher-TF trend
    # as of the session start (00:00 NY) -- no counter-trend trades.
    session_start_ts = datetime(d.year, d.month, d.day, tzinfo=NY)
    trend = trend_as_of(trend_lookup, higher_tf_bars, session_start_ts)
    if trend is None:
        skip(f"{session_label}:no_trend_warmup")
        return None
    if (direction == "LONG" and trend != "UP") or (direction == "SHORT" and trend != "DOWN"):
        skip(f"{session_label}:counter_trend")
        return None
    ...
    # WIDER SL variant: use the candle BEFORE the displacement candle -- i.e.
    # the FVG's own FIRST/start candle (context[fvg.start_index], the candle
    # immediately preceding displacement_bar = context[fvg.start_index + 1])
    # -- instead of the displacement candle itself, giving more room before
    # a stop-out.
    pre_displacement_bar = context[fvg.start_index]
    if direction == "LONG":
        stop = pre_displacement_bar.low
    else:
        stop = pre_displacement_bar.high
...
def run_backtest() -> list[Trade]:
    ...
    higher_tf_bars = build_higher_tf_bars(bars, TREND_TF_MINUTES)
    trend_lookup = build_trend_lookup(higher_tf_bars, TREND_SMA_PERIOD)
    print(f"Trend TF: {TREND_TF_MINUTES}min, {len(higher_tf_bars)} bars, SMA({TREND_SMA_PERIOD})")
