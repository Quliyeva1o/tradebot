...
def test_live_trades_are_printed_and_counted_without_backtest_data(capsys):
    ...
    n, pnl = report._report_bot("NDX100", "Breakout", "30m OR / M5 / 4R", RISK_PCT, ROWS, base={})
...
def test_no_verdict_is_given_without_a_baseline_to_compare_against(capsys):
    ...
    report._report_bot("NDX100", "Breakout", "30m OR / M5 / 4R", RISK_PCT, [dict(ROWS[0]) for _ in range(6)], base={})
...
def test_bot_with_a_baseline_and_no_trades_adds_nothing_to_the_total(capsys):
    ...
    n, pnl = report._report_bot("SPX500", "Breakout", "60m OR / M1 / 4R", RISK_PCT, [], base=BASE)
