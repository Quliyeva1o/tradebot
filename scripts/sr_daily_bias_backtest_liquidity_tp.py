"""Variant of scripts/sr_daily_bias_backtest.py: TP is no longer a fixed 3R --
instead it's the nearest UNMITIGATED opposite-side swing level (the next
Resistance above entry for LONG, the next Support below entry for SHORT) --
i.e. a liquidity-pool target, same concept as scripts/bias_liquidity_backtest.py's
nearest_unmitigated_swing() but computed on THIS strategy's own intraday
swing pivots instead of daily ones. If no unmitigated opposite-side level
exists, or the resulting reward is too small, the trade is skipped (no
fallback to 3R -- the user asked to REPLACE the 3R target, not supplement it).
Everything else (bias, S/R, bounce/breakout/retest entries, SL placement,
filters) is unchanged from sr_daily_bias_backtest.py.
...
TRADES_OUT = f"artifacts/sr_daily_bias_liquidity_tp_trades_{TIMEFRAME_MINUTES}m.csv"
MIN_REWARD_ATR = 0.5  # skip the trade if the nearest liquidity target gives less reward than this
...
def run_backtest(tf_minutes: int, input_csv: str = INPUT_CSV) -> list[Trade]:
    ...
    # Liquidity-pool tracking: every confirmed swing high/low stays "active"
    # until price trades through it (mitigated), same concept as
    # scripts/bias_liquidity_backtest.py's nearest_unmitigated_swing() but
    # kept as simple live lists here since this runs bar-by-bar anyway.
    ...
    for i in range(warmup, n):
        ...
        # Mitigate: any active level price has now traded through is no
        # longer a valid liquidity target.
        ...
        retest_long_level, retest_short_level = broken_res_level, broken_sup_level  # captured BEFORE consuming below
        retest_long = bias == 1 and broken_res_bar is not None and l[i] <= retest_long_level + TOUCH_TOLERANCE_ATR * atr[i] and l[i] >= retest_long_level - TOUCH_TOLERANCE_ATR * atr[i] and bull_rej
        retest_short = bias == -1 and broken_sup_bar is not None and h[i] >= retest_short_level - TOUCH_TOLERANCE_ATR * atr[i] and h[i] <= retest_short_level + TOUCH_TOLERANCE_ATR * atr[i] and bear_rej
        if retest_long:
            broken_res_level, broken_res_bar = np.nan, None
        if retest_short:
            broken_sup_level, broken_sup_bar = np.nan, None
        ...
        if long_setup:
            ...
            liq_target = min(liq_targets) if liq_targets else None
            if MIN_RISK_ATR * atr[i] <= risk_dist <= MAX_RISK_ATR * atr[i] and liq_target is not None and (liq_target - c[i]) >= MIN_REWARD_ATR * atr[i]:
                entry_type = "Bounce" if long_bounce else ("Breakout" if bullish_breakout else "Retest")
                in_position = True
                pos_dir, pos_sl, pos_tp = "LONG", sl_price, liq_target
                pos_entry_time, pos_entry_price, pos_entry_type = idx[i], c[i], entry_type
            elif liq_target is None:
                skip("no_liquidity_target")
            elif not (MIN_RISK_ATR * atr[i] <= risk_dist <= MAX_RISK_ATR * atr[i]):
            ...
            liq_target = max(liq_targets) if liq_targets else None
            if MIN_RISK_ATR * atr[i] <= risk_dist <= MAX_RISK_ATR * atr[i] and liq_target is not None and (c[i] - liq_target) >= MIN_REWARD_ATR * atr[i]:
                entry_type = "Bounce" if short_bounce else ("Breakout" if bearish_breakout else "Retest")
                in_position = True
                pos_dir, pos_sl, pos_tp = "SHORT", sl_price, liq_target
                pos_entry_time, pos_entry_price, pos_entry_type = idx[i], c[i], entry_type
            elif liq_target is None:
                skip("no_liquidity_target")
            elif not (MIN_RISK_ATR * atr[i] <= risk_dist <= MAX_RISK_ATR * atr[i]):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tf", type=int, default=TIMEFRAME_MINUTES)
    args = parser.parse_args()

    input_csv = args.input_csv or f"data/history/{args.symbol}_M1.csv"
    trades_out = f"artifacts/sr_daily_bias_liquidity_tp_trades_{args.symbol}_{args.tf}m.csv"

    trades = run_backtest(args.tf, input_csv=input_csv)
