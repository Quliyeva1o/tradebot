...
def paper_trades_from_state(path: Path) -> list[PaperTrade]:
    ...
    for o in orders:
        request = o["request"]
        if o["status"] != "FILLED" or request["order_type"] not in ("BUY_LIMIT", "SELL_LIMIT"):
            continue
...
def main() -> None:
    ...
    first_day = max(m1[0].timestamp, m15[0].timestamp).astimezone(NY).date() + timedelta(days=1)
    backtest = [t for t in run_backtest(m15, m1, cfg, args.symbol, spread) if t.day >= first_day]
    paper = [t for t in (paper_trades_from_state(state) if state.exists() else [])
             if t.entry_time.astimezone(NY).date() >= first_day]
    rows = compare(backtest, paper)

    print(f"{args.symbol} First FVG window paritet, {first_day} -> bu gun, spread {spread:.2f}")
